...
import { useWindowBus } from "../desktop/WindowBus";
...
// JSON z CMS může mít neúplné položky → dovolíme Partial
type BlogJson = { items?: Array<Partial<BlogPost>> };

/* ===== Fallback demo příspěvky (když v JSON nic není) ===== */
...
function detectLang(t: (k: string) => string): "cs" | "en" {
  const candidates = [t("lang"), t("locale"), t("i18n.lang")].map(v => (v || "").toLowerCase());
...
const BlogSection: React.FC<Props> = ({ t }) => {
  const { openPostWindow } = useWindowBus();

  const lang = useMemo(() => detectLang(t), [t]);
  const [posts, setPosts] = useState<BlogPost[]>(FALLBACK_POSTS);

  // Dynamický import JSON podle jazyka (menší bundle)
  useEffect(() => {
    let active = true;
    async function load() {
      try {
        const mod: BlogJson =
          lang === "cs"
            ? ((await import("../../data/i18n/cs.blog.json")).default as BlogJson)
            : ((await import("../../data/i18n/en.blog.json")).default as BlogJson);

        if (active) {
          const items = normalizeItems(mod);
          setPosts(
            (items.length ? items : FALLBACK_POSTS).sort(
  ...
  return (
    <div className="w-full h-full p-3">
      <div className="w-full h-full flex flex-col md:flex-row gap-3">
        {/* VLEVO: Ikony souborů (poznámkový blok) */}
          ...
          <ul className="divide-y divide-black/20 max-h-[calc(100%-40px)] overflow-auto text-[15px] leading-6">
            {posts.map((p) => (
              <li
                ...
                onClick={() => openPostWindow(p)} // ✅ otevře jako okno
                onKeyDown={(e) => e.key === "Enter" && openPostWindow(p)}
                role="button"
                aria-label={`Otevřít ${p.title}`}
              >
